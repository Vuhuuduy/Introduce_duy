...
const payment = json_data['payment']
...
$(document).ready(function() {
    ...
    for (let i = 0; i < payment.length; i++) {
        ...
        <div class="link-item">
            <div class="bio-story-thumb"
            style="background-image: url('./public/images/flatform/${payment[i]['image']}');"
            ></div>
            <div class="link-content">
                <p>${payment[i]['title']}</p>
                <span
                onclick="${payment[i]['type'] == 0 ? `window.open(${"'"}${payment[i]['value']}${"'"}, '_blank')` : `copy(${"'"}${payment[i]['value']}${"'"})`}"
                >${payment[i]['show-value']}</span>
            </div>
            <div class="link-btn">
                <div class="link-btn-chill ${payment[i]['type'] == 0 ? 'light-orange' : 'light-blue'}"
                onclick="${payment[i]['type'] == 0 ? `window.open(${"'"}${payment[i]['value']}${"'"}, '_blank')` : `copy(${"'"}${payment[i]['value']}${"'"})`}"
                >${payment[i]['type'] == 0 ? 'OPEN' : 'COPY'}</div>
            </div>
        </div>
        `;
    }
